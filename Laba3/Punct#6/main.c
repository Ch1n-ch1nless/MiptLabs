...
int main()
{
    const char* dir_path = VERY_SMALL_TESTS_PATH;
    size_t from = 0;
    size_t to   = 150;
    size_t step = 1;
    ...
    FILE* time_list = fopen("TimeFiles/quick_sort_block_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, QuickSort);
    fclose(time_list);

    time_list = fopen("TimeFiles/shell_sort_block_time.csv", "w");
