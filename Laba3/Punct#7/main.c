...
int main()
{
    ...
    FILE* time_list = fopen("TimeFiles/intro_sort_11_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, 11, IntroSort);
    fclose(time_list);

    time_list = fopen("TimeFiles/intro_sort_12_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, 12, IntroSort);
    fclose(time_list);

    time_list = fopen("TimeFiles/intro_sort_13_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, 13, IntroSort);
    fclose(time_list);

    time_list = fopen("TimeFiles/intro_sort_14_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, 14, IntroSort);
    fclose(time_list);

    time_list = fopen("TimeFiles/intro_sort_15_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, 15, IntroSort);
    fclose(time_list);

    time_list = fopen("TimeFiles/intro_sort_16_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, 16, IntroSort);
    fclose(time_list);

    time_list = fopen("TimeFiles/intro_sort_17_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, 17, IntroSort);
    fclose(time_list);

    time_list = fopen("TimeFiles/intro_sort_18_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, 18, IntroSort);
    fclose(time_list);

    time_list = fopen("TimeFiles/intro_sort_19_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, 19, IntroSort);
    fclose(time_list);

    time_list = fopen("TimeFiles/intro_sort_20_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, 20, IntroSort);
    fclose(time_list);
                               ...
                               void (*sort_function) (int* const array, const size_t array_size, const size_t depth_limit))
{
    ...
    for (size_t cur_size = from; cur_size <= to; cur_size += step)
    {
        for (size_t k = 0; k < number_of_repetitions; k++)
        {
            ...
            size_t log_n = (size_t) ceil(log10(cur_size + .0));

            double time = 0;
            clock_t time_start = clock();
            if (cur_size != 0)
                sort_function(array, cur_size, depth_limit * log_n);
