...
int main()
{
    ...
    FILE* time_list = fopen("TimeFiles/iter_merge_sort_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, IterativeMergeSort);
    fclose(time_list);

    time_list = fopen("TimeFiles/rec_merge_sort_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, RecursiveMergeSort);
    fclose(time_list);
                               ...
                               void (*sort_function) (int* const array, const size_t array_size))
{
    ...
    for (size_t cur_size = from; cur_size <= to; cur_size += step)
    {
        for (size_t k = 0; k < number_of_repetitions; k++)
        {
            ...
            time = (double)(time_end - time_start) / (CLOCKS_PER_SEC / 1000);
