...
{
    const char* dir_path = SMALL_TESTS_PATH;
    size_t from = 0;
    size_t to   = 1000;
    size_t step = 50;

    size_t number_of_repetitions = 5;

    FILE* time_list = fopen("TimeFiles/bubble_sort_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, BubbleSort);
    fclose(time_list);

    time_list = fopen("TimeFiles/insertion_sort_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, InsertionSort);
    fclose(time_list);

    time_list = fopen("TimeFiles/selection_sort_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, SelectionSort);
    fclose(time_list);

    time_list = fopen("TimeFiles/shell_sort_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, ShellSort);
    fclose(time_list);
                               ...
                               void (*sort_function) (int* const array, const size_t array_size))
{
    ...
    for (size_t cur_size = from; cur_size <= to; cur_size += step)
    {
        ...
        time_array[index].time /= number_of_repetitions;
