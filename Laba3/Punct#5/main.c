...
int main()
{
    ...
    FILE* time_list = fopen("TimeFiles/central_pivot_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, QuickSortWithCentralPivot);
    fclose(time_list);

    time_list = fopen("TimeFiles/median_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, QuickSortWithMedianOf3Elements);
    fclose(time_list);

    time_list = fopen("TimeFiles/random_pivot_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, QuickSortWithRandomPivot);
    fclose(time_list);

    time_list = fopen("TimeFiles/median_of_3_random_elems_time.csv", "w");
    TestCorrectnessOfSortings(dir_path, from, to, step, time_list, number_of_repetitions, QuickSortWithMedianOf3RandomElements);
    fclose(time_list);
