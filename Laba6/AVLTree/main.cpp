...
static double   InsertElements(AVLTree* avl_tree, int* array_of_elems, int number_of_elems);
static double   DeleteElements(AVLTree* avl_tree, int* array_of_elems, int number_of_elems);
...
int main(int argc, const char* argv[])
{
    ...
    double time = InsertElements(avl_tree, array_of_elems, number_of_elems);

    printf("%d, %lg\n", number_of_elems, time);

    time = DeleteElements(avl_tree, array_of_elems, number_of_elems / 2);
...
static double DeleteElements(AVLTree* avl_tree, int* array_of_elems, int number_of_elems)
{
    ...
    for (int j = 0; j < 5; j++)
    {
        ...
        for (int i = 0; i < number_of_elems; i++)
        {
            AVLTreeDelete(avl_tree, array_of_elems[i]);
        }
        ...
        for (int i = 0; i < number_of_elems; i++)
        {
            AVLTreeInsert(avl_tree, array_of_elems[i]);
        }
