...
static double   InsertElements(BinaryTree* binary_tree, int* array_of_elems, int number_of_elems);
static double   DeleteElements(BinaryTree* binary_tree, int* array_of_elems, int number_of_elems);
...
int main(int argc, const char* argv[])
{
    ...
    BinaryTree* binary_tree = BinaryTreeCtor();

    double time = InsertElements(binary_tree, array_of_elems, number_of_elems);

    printf("%d, %lg\n", number_of_elems, time);

    time = DeleteElements(binary_tree, array_of_elems, number_of_elems / 2);

    printf("%d, %lg\n", number_of_elems / 2, time);

    free(array_of_elems);
    BinaryTreeDtor(binary_tree);
...
static double DeleteElements(BinaryTree* binary_tree, int* array_of_elems, int number_of_elems)
{
    ...
    for (int j = 0; j < 5; j++)
    {
        ...
        for (int i = 0; i < number_of_elems; i++)
        {
            BinaryTreeDelete(binary_tree, array_of_elems[i]);
        }
        ...
        for (int i = 0; i < number_of_elems; i++)
        {
            BinaryTreeInsert(binary_tree, array_of_elems[i]);
        }
