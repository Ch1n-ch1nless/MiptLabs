...
static double   InsertElements(SkipList* skiplist, int* array_of_elems, int number_of_elems);
static double   DeleteElements(SkipList* skiplist, int* array_of_elems, int number_of_elems);
...
int main(int argc, const char* argv[])
{
    ...
    SkipList* skiplist = SkipListCtor();

    double time = InsertElements(skiplist, array_of_elems, number_of_elems);

    printf("%d, %lg\n", number_of_elems, time);

    time = DeleteElements(skiplist, array_of_elems, number_of_elems / 2);

    printf("%d, %lg\n", number_of_elems / 2, time);

    free(array_of_elems);
    SkipListDtor(skiplist);
...
static double DeleteElements(SkipList* skiplist, int* array_of_elems, int number_of_elems)
{
    ...
    for (int j = 0; j < 5; j++)
    {
        ...
        for (int i = 0; i < number_of_elems; i++)
        {
            SkipListDelete(skiplist, array_of_elems[i]);
        }
        ...
        for (int i = 0; i < number_of_elems; i++)
        {
            SkipListInsert(skiplist, array_of_elems[i]);
        }
